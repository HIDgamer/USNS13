...
/datum/preferences/proc/ShowChoices(mob/user)
	...
	switch(current_menu)
			...
			if(length(gear))
				...
				for(var/i = 1; i <= length(gear); i++)
					...
					if(G)
						...
						dat += "[G.display_name][fluff_cost][loadout_cost] <a href='byond://?src=\ref[user];preference=loadout;task=remove;gear=[i]'><b>Remove</b></a><br>"
		...
		if(MENU_SETTINGS)
			...
			dat += "<b>Window Scaling:</b> <a href='?_src_=prefs;preference=window_scale'><b>[window_scale ? "Larger windows (default)" : "Smaller zoom"]</b></a><br>"
			...
			dat += "<b>Allow Flashing Lights:</b> <a href='?src=\ref[src];action=proccall;procpath=/client/proc/set_flashing_lights_pref'>Set</a><br>"
			...
			dat += "<b>Xeno Cooldown Messages:</b> <a href='?_src_=prefs;preference=show_cooldown_messages'><b>[(show_cooldown_messages) ? "Show" : "Hide"]</b></a><br>"
		...
		if("loadout")
			switch(href_list["task"])
				if("input")
					...
					if(total_cost <= MAX_GEAR_COST)
						gear += G.display_name
						to_chat(user, SPAN_NOTICE("Added \the '[G.display_name]' for [G.fluff_cost] points ([MAX_GEAR_COST - total_cost] points remaining)."))
					else
